...
for (vol_data,seg_data) in genera.vol_seg(vol_data_dir, seg_data_dir,relabel=labels_data,  nb_labels_reshape=len(labels_data),
    ...
    for i in range(0,191):
        ...
        # concatenate slices
        concatenate_outcome = load_model.predict(slice_vol)
        concatenate_outcome = np.concatenate([concatenate_outcome,concatenate_outcome])

    concatenate_outcome.reshape([1,concatenate_outcome.shape[],concatenate_outcome.shape[0],concatenate_outcome.shape[2],concatenate_outcome.shape[3]])
